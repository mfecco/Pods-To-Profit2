...
public class Weather : MonoBehaviour
{
    ...
    */
    ...
    void Start()
    {
        float[][] phases = { p1temp, p2temp, p3temp, p4temp, p5temp };
        int[] outOfRangeCounts = new int[phases.Length];

        for (int i = 0; i < phases.Length; i++)
        {
            int count = 0;
            foreach (float temp in phases[i])
            {
                if (temp < 68.0f || temp > 86.0f)
                {
                    count++;
                }
            }
            Debug.Log("Out of range phase " + i + ": " + count);
            outOfRangeCounts[i] = count;
        }
